...
# ---------------------------------------------
# Nominal Dynamics (fixed 0.75 mass ratio)
# ---------------------------------------------
...
t_horizon = 1.0
episodes_per_task = 1
ratios = np.round(np.linspace(0.5, 5.0, 50), 3)

all_data = []

for task_id, ratio in enumerate(ratios):
    print(f"[Task {task_id}] Mass ratio = {ratio:.2f}")
    mass_nominal = 0.027*0.5  # default mass from quadrotor class (if not exposed, hardcode it here)
    ...
    for episode_id in range(episodes_per_task):
        ...
        for step in range(Steps):
            ...
            episode_data.append([
                task_id, episode_id, step * dt, *x, *u,
