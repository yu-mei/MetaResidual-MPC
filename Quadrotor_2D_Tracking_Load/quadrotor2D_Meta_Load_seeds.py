...
class Quadrotor2DLearnedDynamics:
    ...
    def model(self):
        ...
        nominal_ratio = 0.5
        m = 0.027 * nominal_ratio
        Iyy = self.gym_env.J[1, 1] 
...
for seed in range(41, 51):
    ...
    # ------------------------------------------------------------------------------
    # Residual MLP: meta
    residual_checkpoint = torch.load("MetaLearning/maml_quadrotor_meta_init_3_64.pth")
    residual_mlp = MLP(
    ...
    for i in range(Steps):
        current_time = i * dt  # current global time

        # === Dynamically update quadrotor mass at half simulation time ===
        if not mass_load and np.isclose(current_time, Tsim / 2, atol=dt):
            pb.changeDynamics(
            ...
            env.MASS *= 1.5  # update internal record
            print(f"\n💡 Quadrotor mass changed to {env.MASS:.3f} kg at t = {current_time:.2f} s\n")
            mass_load = True  # ✅ Prevent future changes

        # Circle parameters
        radius = 0.5  # meters
        center_x = 0.0
        center_z = 1.0
        omega = 2 * np.pi / 15  # One circle every 10 seconds
