...
# Load Meta-Dataset
# -------------------------
csv_path = os.path.join(os.path.dirname(__file__), '../meta_dataset_quadrotor/quadrotor_meta_residual_mpc.csv')
...
for _, row in df.iterrows():
    tasks[row['task_id']].append((
        row['x'], row['x_dot'], row['z'], row['z_dot'], row['theta'], row['theta_dot'],
        row['u1'], row['u2'],
        row['res_x_ddot'], row['res_z_ddot'], row['res_theta_ddot']
    ))
...
for tid, samples in tasks.items():
    ...
    inputs = data[:, :8]   # 6 state + 2 input
    targets = data[:, 8:]  # 3 residual accelerations
...
# -------------------------
class MLP(nn.Module):
    def __init__(self, input_dim=8, output_dim=3, hidden_dim=64, num_layers=2):
        super().__init__()
...
def main():
    ...
    meta_lr = 5e-4
    inner_lr = 5e-3
    meta_batch_size = min(10, len(task_data))
    #meta_batch_size = len(task_data)
    inner_steps = 1
    epochs = 30000
    K = 32  # Support/query set size
    ...
    save_path = os.path.join(save_dir, f"maml_quadrotor_meta_init_{num_layers}_{hidden_dim}.pth")
    torch.save({
    ...
    plt.title(f"MAML Meta-Training Loss (Quadrotor2D Residuals, {epochs} epochs)")
    plt.grid(True)
    plt.yscale("log")
    plt.tight_layout()

    # Save loss plot with same base name as model
    plot_path = save_path.replace(".pth", ".png")
