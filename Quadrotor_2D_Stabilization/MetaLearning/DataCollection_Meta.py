...
Iyy_ratios = np.round(np.linspace(0.75, 2.0, 10), 3)
    ...
    for ratio2 in Iyy_ratios:
        ...
        for episode_id in range(episodes_per_task):
            ...
            for step in range(Steps):
                for k in range(N):
                    y_ref_k = np.array([0, 0, 1, 0, 0, 0, 0, 0])
                    solver.set(k, "yref", y_ref_k)
                y_ref_terminal = np.array([0, 0, 1, 0, 0, 0])
                ...
                episode_data.append([
                    task_id, episode_id, step * dt, ratio1, ratio2,
                    *x, *u,
            ...
            df_ep = pd.DataFrame(episode_data, columns=[
                "task_id", "episode_id", "time", "ratio1", "ratio2",
