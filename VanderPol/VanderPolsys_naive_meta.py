...
residual_checkpoint = torch.load("MetaLearning/maml_vdp_meta_init_2_64.pth")
residual_mlp = MLP(
...
# Output folder
segment_dir = "results/segments_naiveMetamlp"
...
for i in range(Steps):
    ...
    if i > 0 and i % 50 == 0 and len(obs_buffer) >= 50:
        ...
        for _ in range(100):  # few gradient steps
...
# ------------------------------------------------------------------------------
# Compute RMSE for Naive + Meta-Residual MLP
meta_errors = []

for i, seg in enumerate(segments):
    seg_real = real[i * 50: i * 50 + 51, 1:3]  # [x1, x2] ground truth segment
    rmse = np.sqrt(mean_squared_error(seg_real, seg))  # RMSE over all 51×2 entries
    meta_errors.append(rmse)

meta_errors = np.array(meta_errors)

print(f"Naive + Meta-Residual MLP Avg RMSE: {meta_errors.mean():.4f} ± {meta_errors.std():.4f}")

# ------------------------------------------------------------------------------
# RMSE over segments plot
plt.figure(figsize=(8, 3))
plt.plot(meta_errors, marker='s', label="Meta-Residual MLP RMSE", color='C2')
plt.xlabel("Segment Index")
plt.ylabel("RMSE")
plt.title("RMSE per Segment (Meta-Residual)")
plt.legend()
plt.grid(True)
plt.tight_layout()
#plt.show()
